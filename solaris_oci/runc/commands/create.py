...
class Create:
    ...
    def init_parser(parent_parser, command_parser):
        parser = command_parser.add_parser('create',
            formatter_class=argparse.ArgumentDefaultsHelpFormatter,
            description='''The create command creates an instance of a container
    ...
    def __init__(self, options):
        ...
        if not rootfs_path.is_dir():
            ...
            exit(1)
        cmd = [options.runz, 'create', options.container_id, str(bundle_path)]
        if options.debug:
